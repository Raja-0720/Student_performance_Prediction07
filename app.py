...
#     .metric-card.orange { border-color: #f39c12; }
...
#
# if not model_loaded:
#     st.error("⚠️ Model not found! Please run **02_model_training.ipynb** first to generate `models/best_model.pkl`.")
#     st.stop()
#
# # ─── Sidebar: Model Info ───────────────────────────────────────────────────────
# with st.sidebar:
...
#     col1.metric("R² Score", f"{meta['r2']:.4f}")
#     col2.metric("RMSE", f"{meta['rmse']:.3f}")
#     col1.metric("MAE", f"{meta['mae']:.3f}")
#     col2.metric("CV R²", f"{meta['cv_r2']:.4f}")
#
#     st.markdown("---")
#     st.markdown("### 📊 All Models")
#     results_df = pd.DataFrame(meta['all_results'])
#     results_df = results_df.set_index('Model')
#     st.dataframe(results_df[['R²','RMSE','MAE']].style.highlight_max(subset=['R²'], color='#d4edda')
#                                                        .highlight_min(subset=['RMSE','MAE'], color='#d4edda'),
#                  use_container_width=True)
#     st.markdown("---")
#     st.caption("📌 Run notebooks 01 & 02 to retrain the model.")
#
# # ─── Main Form ────────────────────────────────────────────────────────────────
...
# # ─── Prediction ───────────────────────────────────────────────────────────────
# if submitted:
#     # Encode inputs exactly as training
...
#
#     feature_order = meta['features']  # exact order used in training
#
#     input_dict = {
#         'age': age,
#         'gender': gender_enc,
...
#     insights = []
#     if study_hours >= 5:
#         insights.append(("✅", "High study hours (≥5h/day) strongly boost exam scores.", "green"))
#     elif study_hours < 2:
#         insights.append(("⚠️", "Low study hours (<2h/day). Increasing study time may improve scores significantly.", "orange"))
#
#     if attendance_pct >= 85:
#         insights.append(("✅", f"Good attendance ({attendance_pct}%) positively impacts performance.", "green"))
#     elif attendance_pct < 70:
#         insights.append(("⚠️", f"Low attendance ({attendance_pct}%) is a risk factor. Try to attend more classes.", "orange"))
#
#     if mental_health_rating <= 3:
#         insights.append(("❗", "Low mental health score. Consider seeking support — it greatly affects academic performance.", "red"))
#     elif mental_health_rating >= 8:
#         insights.append(("✅", "Excellent mental health supports focused studying.", "green"))
#
#     if sleep_hours < 6:
#         insights.append(("⚠️", f"Low sleep ({sleep_hours}h). Aim for 7–9 hours for optimal cognitive performance.", "orange"))
#
#     if social_media_hours > 4:
#         insights.append(("⚠️", f"High social media usage ({social_media_hours}h/day) can distract from studying.", "orange"))
#
#     if not insights:
#         insights.append(("ℹ️", "Profile looks balanced! Keep up the good habits.", "green"))
#
